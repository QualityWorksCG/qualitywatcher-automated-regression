...
describe('Verify functionality on test suites', () => {
    ...
    it.only('Verify that Admin can create a test suite from predefined template with one test case selected', async () => {
        let title = "Auto_Test_200" + commonUtils.randomString(5);
        let description = "Testing predefined test suite creation with one test case" + commonUtils.randomString(10);
        await testSuitePage.clickNewTestSuiteButton();
        await testSuitePage.createTestSuiteUsingTemplateWithSubsetOfTestCases('E-commerce', 1, title, description)
    ...
    it('Verify that Admin can create test suite from predefined template with all test cases selected', async () => {
        let title = "Auto_Test_300" + commonUtils.randomString(5);
        let description = "Testing predefined test suite creation with all test cases" + commonUtils.randomString(10);
        await testSuitePage.clickNewTestSuiteButton();
        await testSuitePage.createTestSuiteUsingTemplateWithAllTestCases('E-commerce', title, description)
        const titleSelector = testSuitePage.getTableCell(title);
        expect(titleSelector).toBeExisting();
        const descriptionSelector = testSuitePage.getTableCell(description);
        expect(descriptionSelector).toBeExisting() ;
    ...
    it('Verify that Admin cannot create test suite from predefined template with the suite Title blank', async () => {
        let description = "Title should not be blank" + commonUtils.randomString(10);
        await testSuitePage.clickNewTestSuiteButton();
        await testSuitePage.createTestSuiteUsingTemplateWithAllTestCases('E-commerce', "", description)
    ...
    it('Verify that Admin cannot create test suite from predefined template with the suite Description blank', async () => {
        let title = "Description should not be blank" + commonUtils.randomString(5);
        await testSuitePage.clickNewTestSuiteButton();
        await testSuitePage.createTestSuiteUsingTemplateWithAllTestCases('E-commerce', title, "")
